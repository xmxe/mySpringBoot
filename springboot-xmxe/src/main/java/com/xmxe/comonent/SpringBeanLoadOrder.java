...
//		logger.info("BeanPostProcessor前置方法bean==[{}],beanName==[{}]",bean,beanName);
		return bean;
	}
...
//		logger.info("BeanPostProcessor后置方法bean==[{}],beanName==[{}]",bean,beanName);
		return bean;
	}
...
//		logger.info("SmartInitializingSingleton.afterSingletonsInstantiated,此时beanName==[{}],applicationCntext==[{}]",beanName,applicationContext);
		// 第一个参数type表示要查找的bean的类型
		// includeNonSingletons 是否考虑非单例bean
		// allowEagerInit 是否允许提早初始化
		String[] beanDefinitionsNames = applicationContext.getBeanNamesForType(MainService.class,false,true);
