...
public class SpringSchedulerTask {
    ...
    @Scheduled(cron = "0/10 * * * * ?")
    public void cron(){
        System.out.println("cron 每10s执行一次");
    }
    ...
    public void fixedDelay(){
        ...
        System.out.println("fixedDelay(任务执行结束为时间点) 先阻塞10s执行打印 等任务执行完后等待等5s才会执行下一个 此时又阻塞了10s 相当于每隔15s才会打印");
    }
    @Scheduled(initialDelay = 10000, fixedRate = 5000)
    public void fixedRaed(){
        System.out.println("fixedRate(任务执行开始为时间点) initialDelay等待10s后执行 每隔5s执行一次");
    }
